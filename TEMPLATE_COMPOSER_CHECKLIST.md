...
| `plane` | Decodes and runs a small Python script that patches `/app/proxy/Caddyfile` to add a `handle /god-mode { redir * /god-mode/ 301 }` block before the image's own `handle_path /god-mode*` block, then `exec /app/start.sh` to continue the normal boot. **Why:** Plane's own "Get Started" button links to `/god-mode` (no trailing slash), which is a confirmed open upstream bug ([makeplane/plane#9068](https://github.com/makeplane/plane/issues/9068), still unfixed as of v1.4.2) — React Router's basename can't match the URL without the slash, so the page hangs on the loading spinner forever with zero error shown. This redirect fixes it transparently for every deployer without needing to patch or rebuild the image. Idempotent (checks before patching, safe on every restart) and validated against Caddy's own directive-priority semantics (`handle` blocks are matched top-to-bottom, exact-path `handle /god-mode` intercepts before the existing `handle_path /god-mode*` wildcard). The full start command is long (base64-embedded script) — copy it verbatim from this project's actual service settings rather than retyping. |
...
- **"Get Started" / `/god-mode` (instance admin setup) hangs on an infinite loading spinner, everything else works fine:** two distinct causes were found and fixed live during this template's build, in this order:
  1. `ADMIN_BASE_URL`, `SPACE_BASE_URL`, and `APP_BASE_URL` unset — these are separate Django settings from `DOMAIN_NAME`/`WEB_URL` and aren't mentioned anywhere in the AIO image's own README or `variables.env`. `/api/instances/` shows all three as `null` in its `config` object when this is the cause. Fixed by setting all three to the full `https://` public domain (unlike `DOMAIN_NAME`, these need the protocol prefix).
  2. Even with the above fixed, the spinner persisted. Root cause: Plane's own "Get Started" button links to `/god-mode` with no trailing slash, a confirmed **open upstream bug** ([makeplane/plane#9068](https://github.com/makeplane/plane/issues/9068), unfixed as of v1.4.2) — React Router's basename can't match the bare path, so nothing renders and no error surfaces. Only `/god-mode/` (trailing slash) works. Fixed permanently for every deployer via the `plane` service's custom start command (see section 2) — patches the Caddyfile to 301-redirect `/god-mode` → `/god-mode/` before Caddy starts, so the shipped "Get Started" button just works.
